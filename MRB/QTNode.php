...
class QTNode
{
	 ...
	 * @throws \Exception
	 */
	private function getIndexOfChildForAABB(AABB $aabb) {
		if (empty($this->children)) {
			throw new \Exception("Empty children\n");
		}
		for ($i = 0; $i < self::maxChildren; $i++) {
	 ...
	 * @throws \Exception
	 */
	public function addObject(QTObj $qtObj) {
		...
		if (!empty($this->children)) {
			...
			if (count($this->objects) < self::maxObjects) {
				$this->objects[] = $qtObj;
			} else {
				$this->splitNode();
	 ...
	 * @return QTObj[]
	 */
	public function getIntersections(AABB $box) {
		$res = [];
